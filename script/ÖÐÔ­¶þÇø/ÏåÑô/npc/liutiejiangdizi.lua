...
--	end	
	...
	--
	if nDate < 20210504 then
		tinsert(strtab,"Më kho¸ Thiªn Ngo¹i L­u Tinh/UnLockTinhKhac_Menu")
	end
...
function UnLockTinhKhac_Menu()
	Say("<color=green> §Ö tö thî rÌn L­u <color>:  Thiªn Ngo¹i L­u Tinh trong hµnh trang sÏ ®­îc më kho¸ tÊt c¶.<color>"
		, 2
		, "X¸c ®Şnh më kho¸/UnLockTinhKhac"
		, "Ta chØ ghĞ ch¬i/nothing"
		);
end

function UnLockTinhKhac()
	local totalCount = GetItemCount(2,1,30321);
	if totalCount > 0 then
		if DelItem(2,1,30321,totalCount) == 1 then
			AddItem(2,1,30321,totalCount);
			WriteLog("[TNLTVOZ] "..GetName().." Më kho¸ thµnh c«ng TNLT "..totalCount);
			Msg2Player("Më kho¸ thµnh c«ng "..totalCount.." c¸i")
		end
	else
		Msg2Player("B¹n kh«ng cã Thiªn Ngo¹i L­u Tinh trong ng­êi!")
		return 0
	end
end
