...
g_szTitle = "<color=green>Sö gi¶ m«n ph¸i:<color>"
function SelectSay()	
	local tbSayDialog = {};
	local nSaySize = 0;
...
--	end
	...
	if 1 == IsExpSvrSystemOpen() then
		if nRoute <= 0 then
			tinsert(tbSayDialog,"Gia nhËp m«n ph¸i/JoinRoute_UpdateLevel");
		end
		if GetLevel() < 78 then
		...
		tinsert(tbSayDialog,"NhËn 1000 vµng/Get_Money");
		tinsert(tbSayDialog,"Max diem dv/danhvongok");
		tinsert(tbSayDialog,"Max diem su mon/sumonok");
		tinsert(tbSayDialog,"NhËn QuyÕt YÕu/Get_Book_JueYao");
		tinsert(tbSayDialog,"Th¨ng cÊp mËt tÞch ®· trang bÞ/Get_Book_Update");
	end
	
	
	if GetName() == "XoaSupport" then
		tinsert(tbSayDialog,"NhËn 1000 vµng/Get_Money2");
		tinsert(tbSayDialog,"NhËn cang khon phu/Get_CKP");	
		tinsert(tbSayDialog,"NhËn hoi minh LB/Get_HMLB");		
		tinsert(tbSayDialog,"NhËn Ngo¹i trang Off/Select_NTOFF");
		tinsert(tbSayDialog,"Vßng th¸i h­ b¸t qu¸i CT/Select_THCT");
		tinsert(tbSayDialog,"LÖnh bµi s­ m«n/#gmVozItem(2, 0, 206, 4)");
...
--	tinsert(tbSayDialog,"Ta muèn ®æi Kim PhiÕu/RutKimPhieu")	
...
tbCashopItems = {
	[1]={"C©y B¸t Nh· nhá", {2,0,504,1}, 10000, 7},
	[2]={"B¹ch C©u Hoµn", {2,1,270,1}, 1000, 7},
	[3]={"Tam Thanh hoµn", {2,1,1097,1}, 1000, 7},
	[4]={"Lôc ThÇn hoµn", {2,1,1064,1}, 1000, 7},
	[5]={"ThÇn N«ng §¬n", {2,1,343,1}, 50000, 7},
	[6]={"TÈy Tñy ®¬n", {2,0,136,1}, 40000, 7},
	[7]={"Tu Ch©n YÕu QuyÕt", {2,0,554,1}, 10000, 7},
	[8]={"TiÓu Nh©n s©m qu¶", {2,0,552,1}, 10000, 7},
	[9]={"C¬ quan nh©n", {2,1,1011,1}, 100000, 7},
	[10]={"V« NgÊn Thñy", {2,1,503,1}, 10000, 7},
	[11]={"N÷ Oa Tinh Th¹ch", {2,1,504,1}, 190000, 7},
	[12]={"B¸t B¶o TÈy Tñy §¬n", {2,0,138,1}, 190000, 7},
	[13]={"Tñ ¸o thiªn biÕn tiÕu", {2,1,30494,1}, 20000, 30},
	[14]={"Gi¸n §iÖp trang", {2,1,9995,1}, 30000, 7},
	[15]={"§Çu thµnh th­", {2,1,9979,1}, 40000, 7},
	[16]={"§Çu thµnh biÓu", {2,1,9978,1}, 80000, 7},
	[17]={"Qu©n c«ng ch­¬ng", {2,1,9999,1}, 20000, 7},
	[18]={"Huy hiÖu chiÕn c«ng (®¹i)", {2,1,9998,1}, 50000, 7},
	[19]={"B¹ch M·", {0,105,34,1, 1,-1,-1,-1,-1,-1,-1}, 2000, 30},
	[20]={"XÝch Thè", {0,105,9,1, 1,-1,-1,-1,-1,-1,-1}, 2000, 30},
	[21]={"Tö Kim Hång Bao", {2,1,536,1}, 10000, 30},
	[22]={"TiÓu Y Vµng", {2,1,30491,1}, 10000, 7},
	}
function VozCashShop()
	local szSayHead = "Vui lßng chän vËt phÈm cÇn mua."
	local tbSayDialog = {};
...
function BuyCashShopItem(index) 
	...
	if GetCash() < tbItem[3] then
		Talk(1,"","B¹n kh«ng ®ñ tiÒn ®Ó mua mãn nµy");
		return 0
	else
...
function UpLevel73()
	if GetPlayerRoute() ==0 then
		Msg2Player('Vµo ph¸i míi ®­îc dïng chøc n¨ng nµy');
		return
	end
	if GetLevel() < 73 then 
		...
		SaveNow()
		Talk(1,"ExitGame","§· th¨ng cÊp 73.")
		do 
			return
		end
	end;
	
	Msg2Player('CÊp < 73 míi cã thÓ v­ît mèc');
end
...
function UpLevelStep59()
	...
	if GetLevel() < 80 then 
		...
		ModifyExp(-nExp);
		SetLevel(80, 0);	
		Msg2Player('§· th¨ng cÊp 80');
		SaveNow()
		Talk(1,"ExitGame","§· th¨ng cÊp 80.")
		do 
			return
		end
	end;
	
	Msg2Player('CÊp 59 míi cã thÓ v­ît mèc');
end
function Select_THCT()
	gf_AddItemEx2({2,1,475,1}, "¢m Hån Ngo¹i Trang Bao", "ADMIN", "mua");
	gf_AddItemEx2({2,1,476,1}, "¢m Hån Ngo¹i Trang Bao", "ADMIN", "mua");
end
function Select_NTOFF()
	gf_AddItemEx2({2,1,31332,1}, "¢m Hån Ngo¹i Trang Bao", "ADMIN", "mua", 30*24*60*60)
end
...
function join_sl()
	...
	Say(g_szTitle.."Ng­¬i muèn gia nhËp l­u ph¸i g×?", getn(szSay), szSay);
	--SelectSay(szSay);
end;
