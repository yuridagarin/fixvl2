...
function business_entrance()
	AskClientForString("business_entrance_recv", "", 1, 20, "NhËp: th")
	
end

function business_entrance_recv(szKey)
	
	if szKey == 'th' then
		business_entrance_ok();
	else
		Say("Vui lßng nhËp ch÷ th vµo « kiÓm tra", 0);
	end
end

-- Ä¬ÈÏÈë¿Úº¯Êı
function business_entrance_ok()
	-- if CheckXacThuc() == 0 then
		-- Say("Tµi kho¶n ch­a x¸c thùc. Xin thİ chñ h·y dõng tay.", 0)
		-- return
	-- end
	--µÈ¼¶¼ì²â	
...
--====================== Nhiem Vu Thuong Hoi ==================	
	if CFG_NhiemVuThuongHoi == 1 then
		...
		local strTalkTab = {}
		tinsert(strTalkTab,"Hoµng th­îng võa ban chiÕu: Nh©n sÜ nµo gióp søc TriÒu ®×nh sÏ ®­îc träng th­ëng! Mçi ngµy cã thÓ nhËn tèi ®a <color=yellow>20 lÇn<color> nhiÖm vô!")
		tinsert(strTalkTab,"NhiÖm vô lÇn tr­íc ®· hoµn thµnh ch­a? Nhí lµ trong ngµy ph¶i hoµn thµnh <color=green>10 nhiÖm vô<color> nhĞ! NÕu <color=green> hñy 1 lÇn sÏ bŞ trõ ®i 1 lÇn<color> trªn tæng sè nhiÖm vô!")
...
--	end
	...
	if GetLevel() < 54 or GetReputation() < nReput or GetTask(336) < nFaction or GetPlayerRoute() == 0 then
		Say("N¨ng lùc cña ng­¬i cßn kĞm l¾m, ®îi ®Õn <color=yellow>cÊp 54, gia nhËp hÖ ph¸i, cã ®ñ 200 ®iÓm danh väng <color>h·y quay l¹i cèng hiÕn søc lùc còng ch­a muén.", 0)
		return
	end
	local nDate = tonumber(date("%y%m%d"))
...
function hand_up_task()
	...
	if (nTaskFinish == 1) then
		...
		if (nCurDayDoneTimes + 1) == 10 then
			gf_AddItemEx2({2,0,398,1,4}, "Bat nha lon", "Tieu Ngao GH", "mua", 24*60*60)
		end
