...
function main()
	-- do
		-- Say("Chøc n¨ng th­ëng trång B¸t Nh· t¹m thêi ®ãng ®Õn khi cã chØnh söa phï hîp h¬n",0)
		-- return
	-- end
	...
	tinsert(tbSayDialog,"H«m nay ®· nép ®ñ 5 B¸t Nh· Chøng Th­ /already_give_16_banruozhengshu")
...
function already_give_16_banruozhengshu()
	...
	if GetTask(TASK_TUOGUAN_GIVE_BANRUO_COUNT) < 5 then
		Say(g_strNpc.."ThiÕu hiÖp míi nép <color=yellow>"..GetTask(TASK_TUOGUAN_GIVE_BANRUO_COUNT).."/5<color> B¸t Nh· Chøng Th­ trong ngµy. H·y nép thªm ®i!!", 1, "ThËt ng¹i qu¸! Ta sÏ quay l¹i sau./nothing");
		return 0;
	else
...
function get_award(nPage)
	local tb_say = {};
	if nPage == 1 then
    	tinsert(tb_say, "250 ®iÓm tİch lòy – nhËn 2 MËt tŞch s­ m«n 70/#confirm_get_award(1, 250)");
...
--		tinsert(tb_say, "400 ®iÓm tİch lòy – nhËn 2 MËt tŞch s­ m«n cap cÊp 70/#confirm_get_award(6, 400)");
...
--		end
	elseif nIndex == 1  then
		...
		if not t_TUOGUAN_LOW[nRoute] then
			Talk(1, "", g_strNpc.."§¹i hiÖp vÉn ch­a gia nhËp m«n ph¸i nµo c¶, kh«ng thÓ nhËn ®­îc mËt tŞch s­ m«n");
			return 0;
		end
