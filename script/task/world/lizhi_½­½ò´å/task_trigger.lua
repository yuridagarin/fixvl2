...
function Fs_Killed()
    if GetItemCount(2,0,84) >= 1 then   --ÓÐËøÑýïñ
        AddItem(2,0,85,1);
    else
        Msg2Player("Kh«ng cã Táa Yªu Chung kh«ng thÓ thu thËp Oan hån");
        TaskTip("Kh«ng cã Táa Yªu Chung kh«ng thÓ thu thËp Oan hån");
    end;
end;
...
function Sn_Killed()
    if GetTask(TASK_JJC) == 15 then
        if g_probability(20, 100) then
            AddItem(2,0,89,1);
            Talk(1,"","Th­ nµy giao ®Õn Tr­êng Ca M«n kho¶ng 4 trang, mét sè trang r¬i vµo tay ng­êi Miªu.");
    ...
    elseif GetTask(TASK_JJC) == 16 then
        if g_probability(10, 100) then
            ...
            if IsComplete() == 0 then
                Talk(3,"","Th­ nµy xem ra do Tèng Giang viÕt","[§iÒu tra th©n thÕ TrÇn Phi D­¬ng...]","Kh«ng ®­îc! Ta ph¶i <color=yellow>®i L­¬ng S¬n t×m Tèng Giang<color> hái cho ra lÏ!");
                SetTask(TASK_JJC, 17);
